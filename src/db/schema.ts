...
/**
 * Drizzle ORM definition for the `superguide.trips` table.
 *
 * Stores trips generated by users. The full itinerary is persisted as JSON
 * so the day-by-day schedule can be round-tripped without a join table.
 * Cascade-deleted when the owning user is deleted.
 */
export const trips = starterSchema.table(
  "trips",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    user_id: varchar("user_id", { length: 128 })
    ...
    itin: jsonb("itin").notNull(),
    created_at: timestamp("created_at").defaultNow(),
  },
  table => ({
    userIdx: index("superguide_trips_user_idx").on(table.user_id),
  })
);
